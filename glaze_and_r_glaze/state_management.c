...
// State Chunk configuration - REMOVED MACRO, will be command-line arg
// #define TARGET_CHUNK_DATA_BYTES (512 * 1024)   
...
static uint64_t SMALL_ACCOUNT_COUNT = 10000000UL;  // Default value, will be overridden by parameter
...
#define MAX_WRITE_SET_COUNT (2 * BATCH_SIZE) // sender+receiver per transfer
#define MAX_RANGE_SET_COUNT (BATCH_SIZE)     // at most all txs are range sets

typedef struct ChunkSlot {
    uint32_t batch_num;
    uint32_t chunk_idx_in_ring; // Index within the cycle (0 to NUM_STATE_CHUNKS-1)
    int64_t state_data[]; // Variable length array based on ACCOUNTS_PER_STATE_CHUNK
} ChunkSlot;

typedef struct BatchSlot {
    uint32_t batch_num;
    uint32_t tx_count;
    // Original transactions for backward compatibility (not used in recovery)
    Transaction transactions[BATCH_SIZE];
    // Hybrid logging fields:
    WriteSetEntry write_set[MAX_WRITE_SET_COUNT];
    uint32_t write_set_count;
    Transaction range_set_transactions[MAX_RANGE_SET_COUNT];
    uint32_t range_set_count;
} BatchSlot;
...
typedef struct {
    ...
    int64_t *state_chunk_data; // Pointer to dynamically allocated state chunk
...
// Function to allocate ChunkSlot with variable size
ChunkSlot* allocate_chunk_slot(void) {
    size_t slot_size = sizeof(ChunkSlot) + ACCOUNTS_PER_STATE_CHUNK * sizeof(int64_t);
    ChunkSlot *slot = malloc(slot_size);
    return slot;
}
...
static FORCE_INLINE void commit_batch_data_to_log(
 ...
 {
     /* ---- Layout helpers -------------------------------------------------- */
     const size_t chunk_slot_size = sizeof(ChunkSlot) + ACCOUNTS_PER_STATE_CHUNK * sizeof(int64_t);
     ...
     const size_t tx_slot_array_offset        = snapshot_slot_header_offset +
                                                NUM_STATE_CHUNKS * chunk_slot_size;

     const size_t final_snap_offset = snapshot_slot_header_offset +
                                      (size_t)slot_idx_in_cycle * chunk_slot_size;
     ...
     nt_memcpy(snap_dst, temp_snap_slot, chunk_slot_size);
...
static void* commit_worker_func(void* arg) {
    ...
    ChunkSlot *current_task_snap_slot = allocate_chunk_slot(); // Worker's own temp slot for snapshot
    BatchSlot current_task_tx_slot;   // Worker's own temp slot for transactions

    if (!current_task_snap_slot) {
        perror("Failed to allocate chunk slot in worker thread");
        return NULL;
    }
    ...
    while (true) {
        ...
        const size_t chunk_slot_size = sizeof(ChunkSlot) + ACCOUNTS_PER_STATE_CHUNK * sizeof(int64_t);
        const size_t snapshot_slot_header_offset = CHECKPOINT_HEADER_SIZE;
        const size_t tx_slot_array_offset = snapshot_slot_header_offset + NUM_STATE_CHUNKS * chunk_slot_size;

        const size_t final_snap_offset = snapshot_slot_header_offset +
                                         (size_t)local_args_for_current_task.slot_idx_in_cycle * chunk_slot_size;
        const size_t final_tx_offset = tx_slot_array_offset +
        ...
        nt_memcpy(snap_dst, current_task_snap_slot, chunk_slot_size);
        ...
        // Hybrid logging logic:
        uint32_t write_set_count = 0;
        uint32_t range_set_count = 0;
        WriteSetEntry write_set[MAX_WRITE_SET_COUNT];
        Transaction range_set_transactions[MAX_RANGE_SET_COUNT];
        for (size_t k = 0; k < local_args_for_current_task.num_tx_in_batch; ++k) {
            Transaction *tx = &current_task_tx_slot.transactions[k];
            ...
            bool applied = false;
            if (sfunc == 0 && rfunc == 0) {
                // Transfer: apply and log write set
                if (sidx < SMALL_ACCOUNT_COUNT && ridx < SMALL_ACCOUNT_COUNT) {
                    int64_t amt = (int64_t)tx->amount;
                    if (local_args_for_current_task.state_chunk_data[sidx] >= amt) {
                        local_args_for_current_task.state_chunk_data[sidx] -= amt;
                        local_args_for_current_task.state_chunk_data[ridx] += amt;
                        // Log sender
                        if (write_set_count < MAX_WRITE_SET_COUNT) {
                            write_set[write_set_count].address = sidx;
                            write_set[write_set_count].balance = local_args_for_current_task.state_chunk_data[sidx];
                            write_set_count++;
                        }
                        // Log receiver
                        if (write_set_count < MAX_WRITE_SET_COUNT) {
                            write_set[write_set_count].address = ridx;
                            write_set[write_set_count].balance = local_args_for_current_task.state_chunk_data[ridx];
                            write_set_count++;
                        }
                        applied = true;
                    }
                ...
                // Range set: apply and log transaction
                uint64_t start_idx = GET_DATA(tx->sender);
                uint64_t count = GET_DATA(tx->receiver);
                int64_t value_to_set = (int64_t)tx->amount;
                for (uint64_t i = 0; i < count; ++i) {
                    uint64_t current_addr = start_idx + i;
                    if (current_addr < PADDED_ACCOUNT_COUNT) {
                        local_args_for_current_task.state_chunk_data[current_addr] = value_to_set;
                    }
                }
                if (range_set_count < MAX_RANGE_SET_COUNT) {
                    range_set_transactions[range_set_count++] = *tx;
                }
                applied = true;
            }
            if (!applied) {
                tx->amount = 0; // Mark as not applied
            }
        }

        current_task_tx_slot.write_set_count = write_set_count;
        current_task_tx_slot.range_set_count = range_set_count;
        nt_memcpy(current_task_tx_slot.write_set, write_set, write_set_count * sizeof(WriteSetEntry));
        nt_memcpy(current_task_tx_slot.range_set_transactions, range_set_transactions, range_set_count * sizeof(Transaction));
        ...
        if (UNLIKELY(local_args_for_current_task.pagesize <= 0)) {
            ...
            void *sync_start = (void *)((uintptr_t)tx_dst & ~((uintptr_t)local_args_for_current_task.pagesize - 1U));
            const uintptr_t last_byte = (uintptr_t)tx_dst + sizeof(BatchSlot) - 1U;
            ...
            if (UNLIKELY(msync(sync_start, sync_len, MS_SYNC))) {
                ...
                snprintf(err_buf, sizeof(err_buf),
                         ...
                         tx_dst, tx_dst, sync_start, sync_len, local_args_for_current_task.current_batch_num);
                perror(err_buf);
...
int recover_state_from_log(int log_fd, int64_t *restrict state_array_to_recover, int *last_recovered_batch_num) {
    ...
    ChunkSlot *temp_snap_slot = allocate_chunk_slot();
    ...
    size_t chunk_slot_size = sizeof(ChunkSlot) + ACCOUNTS_PER_STATE_CHUNK * sizeof(int64_t);
    ...
    for (uint32_t chunk_k = 0; chunk_k < NUM_STATE_CHUNKS; ++chunk_k) {
        ...
        off_t snap_offset_in_file = snapshot_slot_header_offset + (off_t)chunk_k * chunk_slot_size;
        memset(temp_snap_slot, 0, chunk_slot_size);
        ssize_t read_bytes = pread(log_fd, temp_snap_slot, chunk_slot_size, snap_offset_in_file);

        if (read_bytes == (ssize_t)chunk_slot_size && temp_snap_slot->chunk_idx_in_ring == chunk_k) { // Validate chunk_idx_in_ring
    ...
    for (uint32_t chunk_k = 0; chunk_k < NUM_STATE_CHUNKS; ++chunk_k) {
        ...
        if (read_bytes == (ssize_t)chunk_slot_size && temp_snap_slot->chunk_idx_in_ring == chunk_k) {
    ...
    *last_recovered_batch_num = max_loaded_batch_overall;
    ...
    size_t tx_slot_array_offset = CHECKPOINT_HEADER_SIZE + NUM_STATE_CHUNKS * chunk_slot_size;
    ...
    for (uint32_t currently_processing_chnk = positive_modulo(latest_chunk_idx_overall - 1, NUM_STATE_CHUNKS); 
        ...
        for (uint32_t curr_batch = batch_number_of_chunk[currently_processing_chnk] + 1; 
            ...
            if (read_bytes == (ssize_t)sizeof(BatchSlot)) {
                // Replay write set (for transfers)
                for (uint32_t i = 0; i < temp_tx_slot->write_set_count; ++i) {
                    WriteSetEntry *ws = &temp_tx_slot->write_set[i];
                    if (ws->address < PADDED_ACCOUNT_COUNT) {
                        state_array_to_recover[ws->address] = ws->balance;
                        ++collected_tx_count;
                    }
                }
                // Replay range set transactions
                for (uint32_t i = 0; i < temp_tx_slot->range_set_count; ++i) {
                    Transaction *tx = &temp_tx_slot->range_set_transactions[i];
                    uint64_t start_idx = GET_DATA(tx->sender);
                    uint64_t count = GET_DATA(tx->receiver);
                    int64_t value_to_set = (int64_t)tx->amount;
                    for (uint64_t j = 0; j < count; ++j) {
                        uint64_t current_addr = start_idx + j;
                        if (current_addr < PADDED_ACCOUNT_COUNT) {
    ...
    printf("Recovery Step 2: Applied %u log entries. State now reflects batch %d.\n", collected_tx_count, *last_recovered_batch_num);
...
// Function to initialize global parameters based on account count and chunk size
void initialize_parameters(uint64_t account_count, uint64_t target_chunk_bytes) {
    SMALL_ACCOUNT_COUNT = account_count;
    if (target_chunk_bytes == 0) {
        fprintf(stderr, "Error: Target chunk data bytes cannot be zero.\n");
        exit(EXIT_FAILURE);
    }
    ACCOUNTS_PER_STATE_CHUNK = target_chunk_bytes / ACCOUNT_SIZE;
    if (ACCOUNTS_PER_STATE_CHUNK == 0) {
        fprintf(stderr, "Error: ACCOUNTS_PER_STATE_CHUNK is zero, target_chunk_bytes (%lu) is too small for even one account (ACCOUNT_SIZE %d).\n", target_chunk_bytes, ACCOUNT_SIZE);
        exit(EXIT_FAILURE);
    }
    NUM_STATE_CHUNKS = (SMALL_ACCOUNT_COUNT + ACCOUNTS_PER_STATE_CHUNK - 1) / ACCOUNTS_PER_STATE_CHUNK;
    PADDED_ACCOUNT_COUNT = NUM_STATE_CHUNKS * ACCOUNTS_PER_STATE_CHUNK;
    
    printf("Initialized parameters for %lu accounts with target_chunk_bytes %lu:\n", SMALL_ACCOUNT_COUNT, target_chunk_bytes);
    printf("  ACCOUNTS_PER_STATE_CHUNK: %u\n", ACCOUNTS_PER_STATE_CHUNK);
    printf("  NUM_STATE_CHUNKS: %lu\n", NUM_STATE_CHUNKS);
    printf("  PADDED_ACCOUNT_COUNT: %lu\n", PADDED_ACCOUNT_COUNT);
}

void print_usage(const char *program_name) {
    printf("Usage: %s <num_accounts> <chunk_size_kb> [saveref]\n", program_name);
    printf("  num_accounts:   Number of accounts to simulate (e.g., 500000000)\n");
    printf("  chunk_size_kb:  Target data size per state chunk in KB (e.g., 512 for 512KB)\n");
    printf("  saveref:        Optional flag to run in reference mode\n");
    printf("\nExample:\n");
    printf("  %s 1000000 512        # Run with 1M accounts, 512KB chunks\n", program_name);
    printf("  %s 1000000 256 saveref # Run with 1M accounts, 256KB chunks, reference mode\n", program_name);
}

// --- Main Routine ---
int main(int argc, char **argv) {
    if (argc < 3) { // program_name, num_accounts, chunk_size_kb are mandatory
        print_usage(argv[0]);
        exit(EXIT_FAILURE);
    }
    ...
    uint64_t account_count_cmd = strtoull(argv[1], &endptr_accounts, 10);
    if (*endptr_accounts != '\0' || account_count_cmd == 0) {
        fprintf(stderr, "Error: Invalid number of accounts: %s\n", argv[1]);
        print_usage(argv[0]);
        exit(EXIT_FAILURE);
    }

    // Parse chunk size in KB
    char *endptr_chunk_kb;
    uint64_t chunk_size_kb_cmd = strtoull(argv[2], &endptr_chunk_kb, 10);
    if (*endptr_chunk_kb != '\0' || chunk_size_kb_cmd == 0) {
        fprintf(stderr, "Error: Invalid chunk size (KB): %s\n", argv[2]);
        print_usage(argv[0]);
        exit(EXIT_FAILURE);
    }
    uint64_t target_chunk_data_bytes_cmd = chunk_size_kb_cmd * 1024;
    if (target_chunk_data_bytes_cmd < ACCOUNT_SIZE) {
        fprintf(stderr, "Error: Chunk size (%lu bytes) must be at least ACCOUNT_SIZE (%d bytes).\n", target_chunk_data_bytes_cmd, ACCOUNT_SIZE);
        exit(EXIT_FAILURE);
    }
    
    // Initialize parameters based on command line arguments
    initialize_parameters(account_count_cmd, target_chunk_data_bytes_cmd);
    
    bool is_reference_run = false;
    if (argc > 3 && strcmp(argv[3], "saveref") == 0) { // saveref is now argv[3]
    ...
    // Calculate log file size with variable chunk slot size
    const size_t chunk_slot_size = sizeof(ChunkSlot) + ACCOUNTS_PER_STATE_CHUNK * sizeof(int64_t);
    const size_t SINGLE_CYCLE_SNAPSHOT_BYTES = NUM_STATE_CHUNKS * chunk_slot_size;
    ...
    // Allocate state chunk data for global commit args
    global_commit_args_for_worker.state_chunk_data = malloc(ACCOUNTS_PER_STATE_CHUNK * sizeof(int64_t));
    if (!global_commit_args_for_worker.state_chunk_data) {
        perror("Failed to allocate state chunk data for commit worker");
        munmap(mapped_log_region, TOTAL_LOG_FILE_SIZE); close(log_fd_mmap);
        free(main_state_array); free(temp_snap_slot); free(temp_tx_slot);
        exit(EXIT_FAILURE);
    }
    ...
    if (pthread_mutex_init(&commit_mutex, NULL) != 0) {
        ...
        free(global_commit_args_for_worker.state_chunk_data);
    ...
    while(1) {
        ...
        for (size_t k = 0; k < num_tx_read; ++k) {
            ...
            if (sfunc == 0 && rfunc == 0) {
                ...
                if (sidx < SMALL_ACCOUNT_COUNT && ridx < SMALL_ACCOUNT_COUNT) {
                    int64_t amt = (int64_t)tx->amount;
                    if (main_state_array[sidx] >= amt) {
                        main_state_array[sidx] -= amt;
                        main_state_array[ridx] += amt;
                        // Log sender
                        if (write_set_count < MAX_WRITE_SET_COUNT) {
                            write_set[write_set_count].address = sidx;
                            write_set[write_set_count].balance = main_state_array[sidx];
                            write_set_count++;
                        }
                        // Log receiver
                        if (write_set_count < MAX_WRITE_SET_COUNT) {
                            write_set[write_set_count].address = ridx;
                            write_set[write_set_count].balance = main_state_array[ridx];
                            write_set_count++;
                        }
                        applied = true;
                    }
                ...
                for (uint64_t i = 0; i < count; ++i) {
                    uint64_t current_addr = start_idx + i;
                    if (current_addr < PADDED_ACCOUNT_COUNT) {
                        main_state_array[current_addr] = value_to_set;
                    }
                }
                if (range_set_count < MAX_RANGE_SET_COUNT) {
                    range_set_transactions[range_set_count++] = *tx;
        ...
        global_commit_args_for_worker.write_set_count = write_set_count;
        global_commit_args_for_worker.range_set_count = range_set_count;
        nt_memcpy(global_commit_args_for_worker.write_set, write_set, write_set_count * sizeof(WriteSetEntry));
        nt_memcpy(global_commit_args_for_worker.range_set_transactions, range_set_transactions, range_set_count * sizeof(Transaction));
