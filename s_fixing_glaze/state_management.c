...
// --- Operation Encoding ---
// Top 4 bits encode the function; remaining 60 bits hold data.
...
// Preallocate the log file to the expected size.
...
// --- Recovery ---
// Reassemble full state from each checkpoint slot.
...
// Apply a transaction by decoding op codes from sender/receiver.
...
// --- Commit Function ---
// Combines the commit logic from your commit_thread_func with the mmap approach.
// For each batch, commit only the slot corresponding to (batch_num % RING_SIZE).
void commit_slot(uint32_t batch_num, int64_t *state, WriteSetEntry **ws_accum, int *ws_count, void *mapped_region) {
    ...
    header.chunk_offset = slot_index * STATE_CHUNK_COUNT;
    header.state_chunk_count = STATE_CHUNK_COUNT;
    header.write_set_count = ws_count[slot_index];
    
    // Commit the state chunk directly from state.
    memcpy((char*)mapped_region + offset + CHECKPOINT_HEADER_SIZE,
           state + slot_index * STATE_CHUNK_COUNT,
           STATE_CHUNK_SIZE);
    
    // Commit the write-set for this slot.
    memcpy((char*)mapped_region + offset + CHECKPOINT_HEADER_SIZE + STATE_CHUNK_SIZE,
           ws_accum[slot_index],
           WRITE_SET_CHUNK_SIZE);
    
    // Commit the header.
    memcpy((char*)mapped_region + offset,
           &header,
           CHECKPOINT_HEADER_SIZE);
    
    // Reset the write-set counter.
    ws_count[slot_index] = 0;
    
    // Sync this slot.
    msync((char*)mapped_region + offset, CHECKPOINT_SLOT_SIZE, MS_SYNC);
}
...
for (unsigned int batch_num = 0; batch_num < NUMBER_OF_BATCHES; batch_num++) {
    ...
    // Commit all slots in the checkpoint file so the snapshot is consistent.
    for (uint32_t slot = 0; slot < RING_SIZE; slot++) {
        off_t offset = slot * CHECKPOINT_SLOT_SIZE;

        CheckpointHeader header;
        header.magic = CHECKPOINT_MAGIC;
        header.batch_num = batch_num;           // Current batch.
        header.chunk_offset = slot * STATE_CHUNK_COUNT;
        header.state_chunk_count = STATE_CHUNK_COUNT;
        header.write_set_count = ws_count[slot];

            // Copy the state chunk (directly from 'state').
        memcpy((char*)mapped_region + offset + CHECKPOINT_HEADER_SIZE,
                state + slot * STATE_CHUNK_COUNT,
               STATE_CHUNK_SIZE);

            // Copy the write-set for this slot.
        memcpy((char*)mapped_region + offset + CHECKPOINT_HEADER_SIZE + STATE_CHUNK_SIZE,
               ws_accum[slot],
               WRITE_SET_CHUNK_SIZE);
        ...
        // Sync the entire mapped checkpoint region.
    msync(mapped_region, checkpoint_file_size, MS_ASYNC);
