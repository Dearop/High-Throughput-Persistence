...
int main(void) {
    // Define a fixed small set of addresses (for example: 1, 2, ... SMALL_ACCOUNT_COUNT)
    uint64_t addresses[SMALL_ACCOUNT_COUNT];
    for (unsigned i = 0; i < SMALL_ACCOUNT_COUNT; i++) {
        addresses[i] = i + 1;  // or any other scheme to generate unique 64-bit values
    }
    
    // Open the transaction file for writing (this clears any previous data).
