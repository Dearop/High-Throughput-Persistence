...
// Function to initialize global parameters based on account count
void initialize_parameters(uint64_t account_count) {
    SMALL_ACCOUNT_COUNT = account_count;
    ACCOUNTS_PER_STATE_CHUNK = TARGET_CHUNK_DATA_BYTES / ACCOUNT_SIZE;
    if (ACCOUNTS_PER_STATE_CHUNK == 0) {
        fprintf(stderr, "Error: ACCOUNTS_PER_STATE_CHUNK is zero, TARGET_CHUNK_DATA_BYTES is too small for even one account.\n");
        exit(EXIT_FAILURE);
    }
    ...
    printf("Initialized parameters for %lu accounts:\n", SMALL_ACCOUNT_COUNT);
...
void print_usage(const char *program_name) {
    printf("Usage: %s <num_accounts> [saveref]\n", program_name);
    printf("  num_accounts: Number of accounts to simulate (e.g., 500000000)\n");
    printf("  saveref:      Optional flag to run in reference mode\n");
    printf("\nExample:\n");
    printf("  %s 1000000        # Run with 1 million accounts\n", program_name);
    printf("  %s 1000000 saveref # Run with 1 million accounts in reference mode\n", program_name);
}
...
int main(int argc, char **argv) {
    ...
    char *endptr;
    uint64_t account_count = strtoull(argv[1], &endptr, 10);
    if (*endptr != '\0' || account_count == 0) {
        fprintf(stderr, "Error: Invalid number of accounts: %s\n", argv[1]);
        print_usage(argv[0]);
        exit(EXIT_FAILURE);
    }
    
    // Initialize parameters based on account count
    initialize_parameters(account_count);
    
    bool is_reference_run = false;
    if (argc > 2 && strcmp(argv[2], "saveref") == 0) {
